...
function prepareEventFormData() {
  ...
  // Append form data
  formData.append('eventName', eventNameElement.value);
  formData.append('eventDescription', eventDescriptionElement.value);
  formData.append('startDate', startDateElement.value);
  formData.append('address', addressElement.value);
  formData.append('venue', venueElement.value);
  formData.append('eventStatus', eventStatusElement.value);
  
  // Append file data
  formData.append('eventImage', eventImageElement.files[0]);
  formData.append('organizerLogo', organizerLogoElement.files[0]);
  formData.append('venueImage', venueImageElement.files[0]);

  // Append ticket prices (example of adding a JSON structure)
  formData.append('ticketPrices', JSON.stringify(gatherTicketPrices()));

  return formData;
}

function FirsthandleAddEvent(event) {
    event.preventDefault();
